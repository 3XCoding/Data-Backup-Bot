...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("gfile"):
        ...
        if int(clicked) == typed:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    buttons = [[
                      InlineKeyboardButton('🏛 Bharat Torrent Group 🏛', url='https://t.me/BharatTorrentPro')
                      ]]                                        
                except Exception as e:
                    logger.exception(e)
                f_caption = f_caption
            ...
            try:
                if AUTH_CHANNEL and not await is_subscribed(client, query):
                    ...
                    msg1 = await query.message.edit(
                        text=f"<b>Hey 👋{query.from_user.mention}\n<b>📁 Yᴏuʀ Fɪʟᴇ ɪꜱ Rᴇᴀᴅʏ 👇\n\n<b>↬ ғɪʟᴇ ɴᴀᴍᴇ: </b>{title}\n<b>↬ sɪᴢᴇ:</b> {size}\n\n𝘛𝘩𝘪𝘴 𝘍𝘪𝘭𝘦 𝘸𝘪𝘭𝘭 𝘣𝘦 𝘋𝘦𝘭𝘦𝘵𝘦𝘥 𝘪𝘯 {IMDB_DELET_TIME} seconds.!\n©️ Copyright Because the movie will be delete in 5mint.",                                  
                        reply_markup=InlineKeyboardMarkup(
                             [[
                               InlineKeyboardButton("🔗 ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ʟɪɴᴋ 🔗", url = msg.link)
                             ],
                             [
                               InlineKeyboardButton("⚡️ ᴠɪᴇᴡ ꜰɪʟᴇ ᴄʟɪᴄᴋ ʜᴇʀᴇ ⚡️", url = f"{CH_LINK}")
                             ]]
                        ))
        ...
        buttons = [[
            InlineKeyboardButton('⚙️ 𝚂𝚄𝙿𝙿𝙾𝚁𝚃', url='https://t.me/BharatTorrentPro'),
        ...
        buttons = [[
            InlineKeyboardButton('🔆 𝙾𝚁𝙸𝙶𝙸𝙽𝙰𝙻 𝚁𝙴𝙿𝙾 🔆', url='https://github.com/Hey-Adithya/DataBackupBot')
            ],[
            InlineKeyboardButton('🔐 𝙲𝙻𝙾𝚂𝙴', callback_data='close_data'),  
        ...
        buttons = [[
            ...
            InlineKeyboardButton('● 𝙱𝚄𝚃𝚃𝙾𝙽 𝙵𝙾𝚁𝙼𝙰𝚃', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    ...
                    InlineKeyboardButton('✔️ 𝐘𝐄𝐒' if settings["botpm"] else '✖️ 𝐍𝐎',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('𝐅𝐈𝐋𝐄 𝐒𝐄𝐂𝐔𝐑𝐄',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✔️ 𝐘𝐄𝐒' if settings["file_secure"] else '✖️ 𝐍𝐎',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('𝐈𝐌𝐃𝐁', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✔️ 𝐘𝐄𝐒' if settings["imdb"] else '✖️ 𝐍𝐎',
                                         callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('𝐒𝐏𝐄𝐋𝐋 𝐂𝐇𝐄𝐂𝐊',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✔️ 𝐘𝐄𝐒' if settings["spell_check"] else '✖️ 𝐍𝐎',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('𝐖𝐄𝐋𝐂𝐎𝐌𝐄', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✔️ 𝐘𝐄𝐒' if settings["welcome"] else '✖️ 𝐍𝐎',
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📄 𝙿𝙰𝙶𝙴 1/{round(int(total_results) / 6)}", callback_data="pages"),
